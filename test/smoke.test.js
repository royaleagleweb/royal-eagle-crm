/**
 * End-to-end smoke test against a temp database. Exercises the full agency
 * workflow: signup → lead → convert → deal → proposal → send → accept →
 * invoice → payment → expenses → reports.
 *
 * Usage: npm test
 */
process.env.DB_FILE = require('path').join(require('os').tmpdir(), `crm-test-${process.pid}.sqlite`);
...
async function main() {
  ...
  ok('lead converts into contact + company + deal');
  ...
  ok('proposal emails to client contact and is marked sent');
  ...
  ok('payments recorded: partial → paid, overpayment rejected');
  ...
  ok('files upload, list by client, and download round-trip');
...
main()
  .catch((err) => { console.error('\nSMOKE TEST FAILED:', err.message); process.exitCode = 1; })
  .finally(() => {
    server.close();
