...
-- Uploaded documents (contracts, receipts, briefs) stored in the database
-- itself, so backing up crm.sqlite backs up everything.
...
-- Web Push subscriptions registered from the browser (one per device/browser).
...
-- Growth module: goals, habits, and a journal — the same honest-mirror
-- philosophy as the Productivity report, extended to the whole person
-- (business AND personal), not just business follow-through.
CREATE TABLE IF NOT EXISTS goals (
  id            INTEGER PRIMARY KEY AUTOINCREMENT,
  title         TEXT NOT NULL,
  area          TEXT NOT NULL DEFAULT 'other'
                CHECK (area IN ('business', 'health', 'finance', 'relationships', 'growth', 'other')),
  description   TEXT,
  target_date   TEXT,
...
CREATE TABLE IF NOT EXISTS journal_entries (
  ...
  content    TEXT NOT NULL,
  mood       TEXT CHECK (mood IN (NULL, 'great', 'good', 'okay', 'rough', 'bad')),
  owner_id   INTEGER REFERENCES users(id) ON DELETE CASCADE,
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE INDEX IF NOT EXISTS idx_journal_owner_date ON journal_entries(owner_id, entry_date);
